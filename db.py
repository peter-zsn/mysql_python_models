...
class Executer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self.p.last_executed = getattr(self.cursor, 'last_executed', None)
        self.cursor.close()
...
class ConnectionProxy:
    ...
    def execute_many(self, sql, args=None):
        ...
        """
        with Executer(self) as cursor:
            row = cursor.execute_many(sql, args)
        return row
...
class Hub:
    ...
    用法:
    >>> db = Hub(pymysql)
    >>> db.add_pool('default', host='', port=3306, user='', passwd='', db='',
                    charset='utf8', autocommit=True, pool_size=8, wait_timeout=30)
    >>> db.default.auth_user.get(id=1)

    :param driver: MySQLdb, python2 or pymysql python3
    """
    def __init__(self, driver):
        self.pool_manager = mysql_pool.PoolManager(driver)
        self.creators = {}

    def add_pool(self, alias, **connect_kw):
        def creator():
            if 'connect_timeout' not in connect_kw:
                connect_kw['connect_timeout'] = 10
            return self.pool_manager.connect(**connect_kw)
...
class QuerySet:
    ...
    def escape_string(self, s):
        if isinstance(s, unicode):
            charset = self.conn.character_ser_name()
            try:
                s = s.encode(charset)
            except:
                pass
